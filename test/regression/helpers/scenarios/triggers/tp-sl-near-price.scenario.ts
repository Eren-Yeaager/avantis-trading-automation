...
export async function scenarioTPSLNearPrice(
  page: any,
  assetPair: string,
  triggerType: "tp" | "sl",
  priceOffset: number = 0.001
): Promise<boolean> {
  console.log(
    `\n=== Scenario: ${triggerType.toUpperCase()} Near Price (offset: ${
      priceOffset * 100
    }%) ===`
  );
  ...
  recordMetrics({
    testName: `${triggerType.toUpperCase()} Near Price`,
    assetPair,
    executionPrice: triggerPrice,
    oraclePrice: currentPrice,
    priceDifference: Math.abs(triggerPrice - currentPrice),
  });
