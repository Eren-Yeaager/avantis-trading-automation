...
import { testLimitOrderTrigger, testTakeProfitTrigger, testStopLossTrigger } from "../helpers/trigger-tests";
import { REGRESSION_SCENARIOS } from "../regression-config";

const testCon = testWithSynpress(metaMaskFixtures(connectedSetup));

const triggerScenarios = REGRESSION_SCENARIOS.filter(s => s.category === 'trigger');
